#pragma once
#include <algorithm>
#include "../../../../../state/activity/gateway/catalog.h"
#include "../../../../../state/build_data/scenarios/definition.h"
#include "../../../../../middleware/bap/activity_message/sensor_auth_update.h"
namespace dawn::server::bap::encrypted::push::activity::gateway_roster {
namespace native=state::activity::gateway;
...
[[nodiscard]] bool admit(const layouts::Definition& layout,Storage& storage,wire::Roster& roster,FindGroupByKey findByKey,std::uint32_t* failedKey=nullptr) noexcept {
    ...
    if(layout.tag!=native::kScenario || layout.nameLength>layout.name.size()
        ...
        || layout.bubbleCount<=15 || layout.bubbleHashes[15]!=native::kBubbleHash
    ...
    for(const auto& expected:native::kGroups) {
        ...
        for(std::size_t i=0;i<roster.bubbleSubBlocks.size();++i) {
            ...
            if(b.bubble==15) { block=i;++blocks; }
            for(const auto key:b.keys) {
                if(key==expected.key) { if(expected.topLevel || b.bubble!=15) { return false; } ++keys; }
            }
        }
        ...
        keyStorage[before]=expected.key;storage.rosterSubBlocks[block]={15,std::span(keyStorage).first(before+1)};
